...
@Rule(key="OnlyOneReturnClassCheck",
name="Operations should have only one return",
priority= Priority.BLOCKER)
@BelongsToProfile(title=CheckRepository.SONAR_WAY_PROFILE_NAME,priority=Priority.BLOCKER)
@SqaleSubCharacteristic(RulesDefinition.SubCharacteristics.ERRORS)
@SqaleConstantRemediation("30min")

public class OnlyOneReturnClassCheck extends AbstractXmlCheck{
	
	public static final String MESSAGE="Check that the operation can only have a Return";
	


	public void evaluateAttributesClass(Node node){ //Recibe un nodo Clase
