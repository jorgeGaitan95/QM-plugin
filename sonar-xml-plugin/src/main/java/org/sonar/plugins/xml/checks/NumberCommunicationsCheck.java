 ...
 * @author Jorge Gaitán
 */
@Rule(key="NumberCommunicationsCheck",
name="It is not recommended to use many communications",
priority= Priority.INFO)
@BelongsToProfile(title = CheckRepository.SONAR_WAY_PROFILE_NAME, priority = Priority.INFO)
@SqaleSubCharacteristic(RulesDefinition.SubCharacteristics.ERRORS)
@SqaleConstantRemediation("30min")
public class NumberCommunicationsCheck extends AbstractXmlCheck{
	...
	private void ValidateRelationship(Node node){
		...
		for(Node sibling=node.getFirstChild();sibling!= null;sibling=sibling.getNextSibling()){
			...
			{
				contador++;
				if(contador>getVariables().NUMBER_COMMUNICATIONS)
				{
