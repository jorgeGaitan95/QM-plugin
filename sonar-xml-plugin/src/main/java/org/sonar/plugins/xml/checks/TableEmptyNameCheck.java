...
public class TableEmptyNameCheck extends AbstractXmlCheck{
	public static final String MESSAGE="Check that the Table have a Name";
	...
	private void validateTableName(Node node){
		for(Node sibling=node.getFirstChild();sibling!= null;sibling=sibling.getNextSibling()){
			if(sibling.getNodeName().equals(getVariables().NODE_TABLE))
			{
				...
				if(attribute.getNamedItem(getVariables().ATTRIBUTE_NAME_TABLE)!=null){
					String name=attribute.getNamedItem(getVariables().ATTRIBUTE_NAME_TABLE).getNodeValue();
